...
/*
 EL RESULTADO ES MONEDA CELL * NUMERO CELL, Y AL SER PRECIO
 SE ASIGNA AQUI MISMO A MONEDA CELL, LO QUE YA FUNCIONA ES QUE LA VISTA BGHABILITADO
 YA BLOQUEA SELECCIONAR EL RESULTADO DE LA OPERACION.
 EXPECTATIVA -> BUSCAR EN QUE MOMENTO bgHabilitado no está oculta
 DESBLOQUEAR PASOS EXTRA COMO, DONDE ES LA OPERACION
 
 UN EJEMPLO ES QUE EN COMBODINAMICO, SELECCIONE EL PRODUCTO
 Y EL RESULTADO SE ASIGNÓ A TEXTOCELL
 
 */

public class MonedaCell: Cell<String>, CellType, UITextFieldDelegate {
    
    // IBOUTLETS
    @IBOutlet weak var headersView: HeaderView!
    @IBOutlet weak var lblCurrency: UILabel!
    @IBOutlet weak var txtInput: UITextField!
    @IBOutlet weak var lblDivisa: UILabel!
    ...
    public var atributos: Atributos_moneda?
    ...
    public func setObject(obj: Elemento){
        elemento = obj
        atributos = obj.atributos as? Atributos_moneda
        ...
        self.lblDivisa.textColor = .gray
        self.lblCurrency.textColor = .gray
        
        if atributos?.cultura == "es"{
            self.lblDivisa.text =  "MXN"
        }else if atributos?.cultura == "en"{
            self.lblDivisa.text =  "USD"
        }
        ...
        self.headersView.viewInfoHelp = (row as? MonedaRow)?.cell.formCell()?.formViewController()?.tableView
        ...
        self.lblCurrency.leadingAnchor.constraint(equalTo: self.contentView.leadingAnchor, constant: 10).isActive = true
        ...
        self.txtInput.leadingAnchor.constraint(equalTo: self.contentView.leadingAnchor, constant: 25).isActive = true
        self.txtInput.trailingAnchor.constraint(equalTo: self.contentView.trailingAnchor, constant: -55).isActive = true
        
        self.lblDivisa.translatesAutoresizingMaskIntoConstraints = false
        self.lblDivisa.topAnchor.constraint(equalTo: self.headersView.bottomAnchor, constant: 15).isActive = true
        self.lblDivisa.leadingAnchor.constraint(equalTo: self.txtInput.trailingAnchor, constant: 5).isActive = true
    ...
    func formatterCurrency(textField: UITextField){
        ...
        var replace = result?.regexReplace(regEx: "[0-9]*\\.?[0-9]*")
        replace = result?.replacingOccurrences(of: "\(replace ?? "")", with: "")
        if let amountValue = replace, let am = Double(amountValue) {
            var resultFormat = numberFormatter.string(from: am as NSNumber)
            resultFormat?.removeFirst()
            txtInput.text = resultFormat
            row.value = amountValue
        }
    }
    ...
    open func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        if textField.text == "" && row.value != nil && row.value != ""{
            textField.text = row?.value
        }
        formatterCurrency(textField: textField)
    ...
    open func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        // MARK: truncar hasta dos decimales
        let aSet = self.atributos!.decimales != "0" ? NSCharacterSet(charactersIn:"0123456789.").inverted : NSCharacterSet(charactersIn:"0123456789").inverted
        ...
        if (textField.text)!.contains(".")
        {
            ...
            if lastString.count > 1 && !string.isEmpty
            { //Check number of decimal places
                return false
            }
        }
        return string.isEmpty ? true : string == numberFiltered
    }
    ...
    @objc open func textFieldDidChange(_ textField: UITextField) {
        ...
        est!.Resultado = textField.text!.replaceLineBreakEstadistic()
        est!.KeyStroke += 1
...
extension MonedaCell: ObjectFormDelegate{
    ...
    public func setCultura(){
        switch atributos?.cultura {
        case "es2": self.lblCurrency.text = ""; break;
        case "es": self.lblCurrency.text = "$"; break;
        case "en": self.lblCurrency.text = "$"; break;
        case "ja": self.lblCurrency.text = "¥"; break;
        case "cs": self.lblCurrency.text = "Kč"; break;
        case "en-gb": self.lblCurrency.text = "£"; break;
        case "fr": self.lblCurrency.text = "€"; break;
        case "th": self.lblCurrency.text = "฿"; break;
        case "pt-br": self.lblCurrency.text = "R$"; break;
        case "ru": self.lblCurrency.text = "₽"; break;
        default: self.lblCurrency.text = ""; break;
        }
    }
    // MARK: - SET Init Rules
    ...
    public func setEdited(v: String){
        ...
        self.headersView.lblTitle.textColor = UIColor.black
        var valor = v
        if valor.contains(self.lblCurrency.text ?? ""){ valor = valor.replacingOccurrences(of: self.lblCurrency.text ?? "", with: "")}
        txtInput.text = valor
        formatterCurrency(textField: txtInput)
