...
public class SpreadsheetView: UIView {
    ...
    public var visibleCells: [CellSpread] {
        let cells: [CellSpread] = Array(columnHeaderView.visibleCells) + Array(rowHeaderView.visibleCells)
    ...
    private var cellClasses = [String: CellSpread.Type]()
    private var cellNibs = [String: UINib]()
    var cellReuseQueues = [String: ReuseQueue<CellSpread>]()
    ...
    public func register(_ cellClass: CellSpread.Type, forCellWithReuseIdentifier identifier: String) {
    ...
    public func dequeueReusableCell(withReuseIdentifier identifier: String, for indexPath: IndexPath) -> CellSpread {
        if let reuseQueue = cellReuseQueues[identifier] {
            ...
            let reuseQueue = ReuseQueue<CellSpread>()
        ...
        if let nib = cellNibs[identifier] {
            if let cell = nib.instantiate(withOwner: nil, options: nil).first as? CellSpread {
                cell.reuseIdentifier = identifier
                return cell
            }
        }
    ...
    public func cellForItem(at indexPath: IndexPath) -> CellSpread? {
    ...
    public func cellsForItem(at indexPath: IndexPath) -> [CellSpread] {
