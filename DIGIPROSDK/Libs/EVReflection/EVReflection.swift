...
final public class EVReflection {
    ...
    fileprivate static var inicio : String? = nil
    fileprivate static var tag = 0
    ...
    public class func dictionaryFromJson(_ json: String?) -> NSDictionary {
        //print("JSON EVREFLECTION: \(String(describing: json))")
        var Json = json
        if (json == "") && (EVReflectableCode.tag == 1) {
            Json = EVReflectableCode.Code
            EVReflectableCode.tag = 0
        }else if (json != "") && (EVReflectableCode.tag == 1){
            EVReflectableCode.Code = json ?? ""
            EVReflectableCode.tag = 0
        }
        
        let result = NSMutableDictionary()
        if Json == nil {
            evPrint(.IsInvalidJson, "ERROR: The string is nil and is not valid json!")
        ...
        if typeInObject == "Struct" {
            ...
            do {
                if !(value is NSNull) {
                    ...
                    if let emptyArray = setValue as? NSArray {
    ...
    /**
     Filters JSON response to clear any base64 strings.
     - parameter jsonDic:  the json response from the server
     */
    fileprivate static func clearResponseForLogs(jsonDic: NSDictionary) {
        if var logObject = jsonDic["Login"] as? [String:Any] {
            if var splashObject = jsonDic["Splash"] as? [String:Any] {
                
                if splashObject.keys.contains("Splash") {
                    splashObject["Splash"] = ""
                }
                
                if splashObject.keys.contains("Imagen") {
                    splashObject["Imagen"] = ""
                }
                
                if splashObject.keys.contains("NombreImagen") {
                    splashObject["NombreImagen"] = ""
                }
                
                if logObject.keys.contains("Login") {
                    logObject["Login"] = ""
                }
                
                if logObject.keys.contains("Logo") {
                    logObject["Logo"] = ""
                }
                
                var object = jsonDic as! [String:Any]
                
                if object.keys.contains("NombreImagen") {
                    object["NombreImagen"] = ""
                }
                
                object["Login"] = logObject
                object["Splash"] = splashObject
                
            }
        }  else if var object = jsonDic["ReturnedObject"] as? [String:Any],
                   var loginObject = object["Login"] as? [String:Any] {
            
            if loginObject.keys.contains("Logo") {
                loginObject["Logo"] = ""
            }
            
            if object.keys.contains("Imagen") {
                object["Imagen"] = ""
            }
            
            if object.keys.contains("Splash") {
                object["Splash"] = ""
            }
            
            if object.keys.contains("Login") {
                object["Login"] = loginObject
            }
...
open class EVReflectableCode {
    public static var Code: String = ""
    public static var tag : Int = 0
