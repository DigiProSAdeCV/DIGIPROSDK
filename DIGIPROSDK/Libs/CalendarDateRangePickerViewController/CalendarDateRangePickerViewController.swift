...
public class CalendarDateRangePickerViewController: UICollectionViewController {
    ...
    override public func viewDidLoad() {
        ...
        self.title = "Selecciona fechas"
        
        if minimumDate == nil {
            let calendarioAux = Calendar.current
            var minMaxDate = calendarioAux.dateComponents([.day, .month, .year], from: Date())
            minMaxDate.day = 01
            minMaxDate.month = 01
            minMaxDate.year = 2001
            minimumDate = (calendarioAux.date(from: minMaxDate))! as Date
        }
        if maximumDate == nil
        {
            let calendarioAux = Calendar.current
            ...
            minMaxDate.day = 31
            minMaxDate.month = 12
            minMaxDate.year = 2099
            maximumDate = (calendarioAux.date(from: minMaxDate))! as Date
        }
        ...
        let differenceSection = Calendar.current.dateComponents([.month, .day], from: minimumDate, to: selectedStartDate ?? Date())
        let indexNowDate: IndexPath! = NSIndexPath(row: 0, section: differenceSection.month!) as IndexPath
        self.collectionView.scrollToItem(at: indexNowDate, at: .top, animated: true)
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Limpiar", style: .plain, target: self, action: #selector(CalendarDateRangePickerViewController.didTapCancel))
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Guardar", style: .done, target: self, action: #selector(CalendarDateRangePickerViewController.didTapDone))
...
extension CalendarDateRangePickerViewController {
    ...
    override public func numberOfSections(in collectionView: UICollectionView) -> Int {
        if Calendar.current.dateComponents([.month], from: maximumDate) == Calendar.current.dateComponents([.month], from: minimumDate)
        {   return 1    }
        
        let difference = Calendar.current.dateComponents([.month, .day], from: minimumDate, to: maximumDate)
        return (difference.day! + 1) > 0 ? difference.month! + 2 : difference.month! + 1
    }
    ...
    override public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.item < 7 {
            ...
            if isAfer(dateA: date, dateB: maximumDate) {
                cell.disable()
            }
...
extension CalendarDateRangePickerViewController : UICollectionViewDelegateFlowLayout {
    ...
    override public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if isBefore(dateA: cell.date!, dateB: minimumDate) || isAfer(dateA: cell.date!, dateB: maximumDate) {
            return
        }
...
extension CalendarDateRangePickerViewController {
    ...
    func isAfer(dateA: Date, dateB: Date) -> Bool {
