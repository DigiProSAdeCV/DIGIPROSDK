...
///
/// - returns: Promsie
public func Bluebirdreduce<A, B, S: Sequence>(on queue: DispatchQueue = .main, _ items: S, _ initial: B, _ transform: @escaping (B, A) throws -> Promise<B>) -> Promise<B> where S.Iterator.Element == A {
...
extension Promise where Result: Sequence {
    ...
    ///
    /// - returns: Promise
    public func Bluebirdreduce<B>(on queue: DispatchQueue = .main, _ initial: B, _ transform: @escaping (B, Result.Iterator.Element) throws -> Promise<B>) -> Promise<B> {
        return then(on: queue) { results in
            return DIGIPROSDK.Bluebirdreduce(on: queue, results, initial, transform)
        }
    }
}
