...
///
/// - returns: Promise
public func Bluebirdmap<A, B, S: Sequence>(on queue: DispatchQueue = .main, _ items: S, _ transform: @escaping (A) throws -> Promise<B>) -> Promise<[B]> where S.Iterator.Element == A {
    return `try`(on: queue) {
        do {
...
///
/// - returns: Promise
public func BluebirdmapSeries<A, B, S: Sequence>(on queue: DispatchQueue = .main, _ items: S, _ transform: @escaping (A) throws -> Promise<B>) -> Promise<[B]> where S.Iterator.Element == A {
...
extension Promise where Result: Sequence {
    ...
    ///
    /// - returns: Promise
    public func Bluebirdmap<B>(on queue: DispatchQueue = .main, _ transform: @escaping (Result.Iterator.Element) throws -> Promise<B>) -> Promise<[B]> {
        return then(on: queue) { results in
            return DIGIPROSDK.Bluebirdmap(on: queue, results, transform)
        }
    }
    ...
    ///
    /// - returns: Promise
    public func BluebirdmapSeries<B>(on queue: DispatchQueue = .main, _ transform: @escaping (Result.Iterator.Element) throws -> Promise<B>) -> Promise<[B]> {
        return then(on: queue) { results in
            return DIGIPROSDK.BluebirdmapSeries(on: queue, results, transform)
        }
    }
}
